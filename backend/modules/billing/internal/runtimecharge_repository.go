...
type runtimeChargeRepository interface {
	...
	// listSessionRefs 返回该用户分页的开机会话标识（按最近 charge 时间倒序）+ 总会话数。
	listSessions(userID, offset, limit int) ([]string, int64, error)
...
func (r *gormRuntimeChargeRepository) listSessions(userID, offset, limit int) ([]string, int64, error) {
	...
	var total int64
	if err := r.db.Model(&RuntimeCharge{}).
		Where("user_id = ?", userID).
		Distinct("run_session_ref").Count(&total).Error; err != nil {
		return nil, 0, err
	}
	var rows []row
	ord := query.SafeOrder("max_id", "descending", map[string]bool{"max_id": true}, "max_id DESC")
	err := r.db.Model(&RuntimeCharge{}).
