              ...
              <Popover v-if="r.type === 'enum'">
                <PopoverTrigger as-child>
                  <Button variant="outline" size="sm" class="h-8 px-2 text-xs">
                    {{ t('script.params.optionsCol') }}<span class="ml-1 text-muted-foreground">({{ optionsOf(r).length }})</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent class="w-64" align="start">
                  <div class="grid gap-1.5">
                    <Label class="text-xs">{{ t('script.params.optionsCol') }}</Label>
                    <Textarea v-model="r.optionsText" :rows="3" class="text-xs" :placeholder="t('script.params.enumPh')" />
                  </div>
                </PopoverContent>
              </Popover>
            </div>
