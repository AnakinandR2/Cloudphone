...
> ⚠️ **本节已过时（2026-07-02）**：计费/自动化/素材库后端均已建成，**已升级为完整验收**——见
> [TC-10](用例/TC-10-计费与时长到期.md)/[TC-11](用例/TC-11-素材库.md)/[TC-12](用例/TC-12-应用管理S3与按URL安装.md)/[TC-14](用例/TC-14-自动化脚本任务与参数.md)
> 与《[上线验收测试计划总纲](../superpowers/specs/2026-07-02-上线验收测试计划-design.md)》。下列条目仅保留历史语境。

- ~~计费（购买/用量/订单/定价/折扣）~~ → 已建成，见 TC-10。
- ~~自动化（脚本/任务/MCP）~~ → 已建成，见 TC-14。
- ~~素材库~~ → 已建成，见 TC-11；应用库自有 S3/按 URL 安装见 TC-12。
- 组件演示、demo 嵌套菜单 — ⛔/demo（仍为占位）。

原型冒烟对照见 TC-09（已标注取代）。

### 2.3 不在本期测试的能力（缺口）

> 更新（2026-07-02）：**自动化执行引擎**（脚本/任务/计划/参数注入）已建成，见 TC-14；**代理-云手机绑定校验**已补（Plan A Task 5 修复 I2，见 TC-12/安全矩阵）。

以下后端能力仍未实现或未纳入本期：计费双账本对账、欠费保护 cron、换镜像（`updateImage`）等——归后续（Plan D 评估）。
...
- 关联用例 ID，便于回归定位。
- **缺陷双源**：本仓缺陷两个来源汇入本流程——① 内部测试（本计划 §1–§9 + TC-01~09）；② **外部黑盒输入**：独立测试方 cloudphone-tester 经 **GitLab issue**（label `source::cloudphone-tester`）提缺陷，沿用本节「发现→记录→定级→修复→回归→关闭」流程；我方按 **S0–S3** 分级（= 本节 **S1–S4** 的 −1 偏移·我方 S0/S1 = 你们 S1/S2·见 `qa/qa-protocol.md`）；修复后在 issue 评论回 `fix-receipt`（含修复 commit sha），由其用 ≥该 commit 的版本复测后关单（不通过则 reopen 同单追评）。带 `suspect-external` label = 疑外部中台缺陷，归你作为集成方确认归属。外部源不改内部用例与计划，仅作第二缺陷源。**完整流程约束契约见本仓 `qa` 分支 `qa/qa-protocol.md`**；概览见 `docs/测试/外部黑盒缺陷流转.md`。
...
- 上述「无 S1/S2 未关闭缺陷」**含 cloudphone-tester 经 GitLab issue 提交且未关闭的对应等级缺陷**（外部黑盒来源·我方 S0/S1 = 本仓 S1/S2·同等准出门）。
