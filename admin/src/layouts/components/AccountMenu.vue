...
const displayName = computed(() => userStore.name || userStore.account || 'User')

function goHome() {
  router.push(menuStore.firstPath)
}
    ...
    <DropdownMenuTrigger as-child>
      ...
      >
        ...
        <div class="grid min-w-0 flex-1 leading-tight group-data-[collapsible=icon]:hidden">
          ...
          <span class="text-muted-foreground truncate text-xs">{{ userStore.account }}</span>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="grid min-w-0 flex-1 leading-tight">
            <span class="flex items-center gap-1 truncate text-sm font-semibold">
              {{ displayName }}
              <span v-if="userStore.isSuperuser" class="text-muted-foreground text-xs font-normal">
                · {{ t('header.superuser') }}
              </span>
            </span>
            <span class="text-muted-foreground truncate text-xs">[ {{ userStore.account }} ]</span>
